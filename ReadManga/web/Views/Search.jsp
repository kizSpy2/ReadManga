<%-- 
    Document   : Search
    Created on : Oct 25, 2022, 11:31:54 AM
    Author     : ducan
--%>
...
<html>
    <head>
        ...
        <title>search manga by genre</title>
    ...
    <body>
        ...
        <nav>
            <h1>Search Manga</h1>
            <%String name=(String)request.getAttribute("theloaichon");%>
                ...
                <select name = "theloai">
                    <% 
                       for (Map.Entry<Integer, Category> entry : dao.getCategory().entrySet()) {
                           Integer key = entry.getKey();
                           Category val = entry.getValue();%>
                    <option value="<%=val.getCategory_name()%>"  <%=(val.getCategory_name().equals(name) ? "selected" : "")%>><%=val.getCategory_name()%></option>
                    <%}%>
                </select>
            ...
            for (Map.Entry<Integer, Category> entry : dao.getCategory().entrySet()) {
                Integer key = entry.getKey();
                Category val = entry.getValue();
                if(val.getCategory_name().equals(name)){%>
                <a value = "<%=name%>"><%=val.getCategory_infor()%></a><br/>
                <%}}%></h5>
        </nav>
