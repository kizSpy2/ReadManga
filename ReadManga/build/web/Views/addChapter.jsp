<%-- 
    Document   : addChapter
    Created on : Oct 25, 2022, 8:49:31 PM
    Author     : ducan
--%>
...
<html>
    ...
    <body>
        ...
        <nav>
            <i>Thêm Chap Mới</i><br/>
            <h5>Manga</h5><br/><!-- comment -->
            <%String id=(String)request.getAttribute("manga_id_chon");%>
            <form action="addChapter" method="get">
                <select name="manga_id">
                    <% for(Manga manga : dao.getManga()){%>
                    <option value="<%=manga.getManga_id()%>"  <%=((String.valueOf(manga.getManga_id())).equals(id) ? "selected" : "")%>><%=manga.getManga_name()%></option>
                    <%}%>
                </select>
                <%String name=(String)request.getAttribute("trang");%>
                <% for(Chapter chapter : dao.getChap()){%>
                Số chương <a value="<%=chapter.getManga_id()%>"  <%=((String.valueOf(chapter.getManga_id())).equals(name) ? "selected" : "")%>><%=chapter.getChapter_number()%></a>
                <%}%>
            </form>
        </nav>
